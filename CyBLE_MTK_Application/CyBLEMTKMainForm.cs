...
namespace CyBLE_MTK_Application
{
    public partial class CyBLE_MTK : Form
    {
        ...
        public static bool[] shopfloor_permission;
        ...
        public CyBLE_MTK()
        {
            ...
            this.SWVerStatusLabel.Text += System.Reflection.Assembly.GetExecutingAssembly().GetName().Version.ToString();

            shopfloor_permission = new bool[(int)(CyBLE_MTK_Application.Properties.Settings.Default.NumDUTs)];
            ...
            try
            {
                ...
                Logger.PrintLog(this, $"Setting up Current Test Board serial port {CurtBrdSerialPortDialog.DeviceSerialPort.PortName} successfully...", LogDetailLevel.LogRelevant);
            }
            catch (Exception)
            {

                SplashScreen.LoadMessage = $"Setting up Current Test Board serial port {CurtBrdSerialPortDialog.DeviceSerialPort.PortName} failure...";
                Logger.PrintLog(this, $"Setting up Current Test Board serial port {CurtBrdSerialPortDialog.DeviceSerialPort.PortName} failure... " +
                    $"Hint: it is predefined in the AppConfig file, please check it in the device manager and manually connect it. ", LogDetailLevel.LogRelevant);
            }
            SplashScreen.LoadStatus += 8;
            ...
            MTKTestProgram = new TestProgramManager(Logger, MTKSerialPortDialog.DeviceSerialPort, CurtBrdSerialPortDialog.DeviceSerialPort ,DUTSerialPortDialog.DeviceSerialPort);
            ...
            TestProgramRunCycleTimeForBatch = "0.00";
        ...
        {
            if (TestStatus != null && MTKTestProgram.DUTOverallSFCSErrCode != null && MTKTestProgram.DUTTmplSFCSErrCode != null)
            {
        ...
        public bool EditTestProgramDialog(int PreSelectIndex)
        {
            ...
            TestProgramDialog EditTests = new TestProgramDialog(Logger, MTKSerialPortDialog.DeviceSerialPort, CurtBrdSerialPortDialog.DeviceSerialPort, DUTSerialPortDialog.DeviceSerialPort);
        ...
        {
            if (MTKTestProgram.CurrentMTKTestType != MTKTestType.MTKTestProgramAll)
            {
                SetDevErrStat(MTKTestProgram.CurrentDUTTestError, CurrentDUT);
            }
            ...
            if (CyBLE_MTK_Application.Properties.Settings.Default.AutoLogTestsSetting == "DUT" && !Ignore)
            {
                ...
                else
                {
                    ...
                    {
                        MTKTestProgram_OnShopfloorPermissionCheckFail(CurrentDUT);
                    }
                    else
                    {
        ...
        {
            if ((err == MTKTestError.TestFailed || err == MTKTestError.ProcessCheckFailure) && DUTsTestFlag[DevNum])
            {
                if (!CyBLE_MTK.shopfloor_permission[DevNum])
                {
                    MTKTestProgram_OnShopfloorPermissionCheckFail(DevNum);
                }
                else
                {
        ...
        private void MTKTestProgram_OnShopfloorServerConnectionCheckFail(int CurrentDUT)
        {
            ProgramStatus[MTKTestProgram.CurrentDUT] = MTKTestError.TestFailed;
            this.Invoke(new MethodInvoker(() => DUTInfoDataGridView.Rows[CurrentDUT].Cells["Status"].Style = new DataGridViewCellStyle { ForeColor = Color.DarkRed, BackColor = Color.Pink }));
            this.Invoke(new MethodInvoker(() => DUTInfoDataGridView.Rows[CurrentDUT].Cells["Status"].Value = "SFCS LOSS"));
        ...
        private void MTKTestProgram_OnOverallFail()
        {
            ...
            if (IsProgAllPresent() && (ProgAllErr.Count > 0))
            {
                ...
                for (int i = 0; i < ProgAllErr.Count; i++)
                {
                    ...
                    {
                        if ((!DUTsTestFlag[i] && CyBLE_MTK.shopfloor_permission[i])||ProgAllErr[i] == MTKTestError.IgnoringDUT || ProgAllErr[i] == MTKTestError.ProgrammerNotConfigured)
                        {
                        ...
                        {
                            if (!CyBLE_MTK.shopfloor_permission[i])
                            {
                                ProgramStatus[i] = MTKTestError.ProcessCheckFailure;
                                MTKTestProgram_OnShopfloorPermissionCheckFail(i);
                            }
                            else
                            {
            ...
            else
            {
                ...
                {
                    if ((ProgramStatus[i] == MTKTestError.TestFailed || ProgramStatus[i] == MTKTestError.ProcessCheckFailure) && DUTsTestFlag[i] )
                    {
        ...
        private void MTKTestProgram_OnOverallPass()
        {
            if (IsProgAllPresent())
            {
                ...
                for (int i = 0; i < ProgAllErr.Count; i++)
                {
                    ...
                    if (ProgAllErr[i] == MTKTestError.IgnoringDUT || ProgAllErr[i] == MTKTestError.ProgrammerNotConfigured || (!DUTsTestFlag[i]&& CyBLE_MTK.shopfloor_permission[i]))
                    {
                        ProgramStatus[i] = MTKTestError.IgnoringDUT;
                    }
                }
                    ...
                    if (!MTKTestProgram.SupervisorMode && (CyBLE_MTK_Application.Properties.Settings.Default.TestModeLongRun == 0 || CyBLEMTKRobotServer.IsGlobalServerActive()) )
                    {
                        ...
                        try
                        {
                            ...
                            else
                            {
                                ...
                                m_SFCS.LastError = "Fail to Connect Shopfloor Server...";
                                for (int i = 0; i < DutCount; i++)
                                {
                                    MTKTestProgram_OnShopfloorServerConnectionCheckFail(i);
                                }
                        ...
                        try
                        {
                            if (isSQLconnected)
                            {
                                ...
                                TestThread.Start();
                                Logger.PrintLog(this, $"Test Thread {TestThread.Name} is Started", LogDetailLevel.LogRelevant);
                            }
                            else
                            {
                                if (MTKTestProgram.DUTOverallSFCSErrCode == null)
        ...
        public bool Connect2CurtBrd(SerialPort SPort)
        {
            bool retVal = false;
            ...
            try
            {
                SPort.Open();
                List<string> Rets;
                if (HCISupport.Who(SPort, Logger, this, out Rets))
                {
                    if (Rets[0] == "HOST" &&
                        Rets[1] == "819")
                    {
        ...
        public void LoadTest(bool LoadFromPath, string LoadFilePath)
        {
            ...
            if (MTKTestProgram.LoadTestProgram(LoadFromPath, LoadFilePath) == true)
            {
                ...
                CurrentTestXMLFile = MTKTestProgram.TestFileName;
            }
        ...
        #region SFCS control
        //==============================
        //  SFCS control   shopfloor modified by cysp
        //==============================
        public int CheckPemission()
        {
            int retVal = -1;
            //CyBLE_MTK_Application.Properties.Settings.Default.Reset();
            //CyBLE_MTK_Application.Properties.Settings.Default.Reload();
            ...
            for (int i = 0; i < DUTInfoDataGridView.Rows.Count; i++)
            {
                ...
                {
                    SerialNumber = "AUTO_000000000" + AUTO_SN_index.ToString();
                    DUTInfoDataGridView.Rows[i].Cells["Unique ID"].Value = SerialNumber;
                    AUTO_SN_index++;
                }
                ...
                try
                {
                    permission_info = m_SFCS.PermissonCheck(SerialNumber, "123456", "12345", "MTK");

                }
                catch (Exception ex)
                {
                    MessageBox.Show(m_SFCS.GetType().ToString().Substring(22) + " MES ProcessCheck ERROR (ErrCode: 404): " + ex.Message);
                    Logger.PrintLog(this, m_SFCS.GetType().ToString().Substring(22) + " MES ProcessCheck ERROR (ErrCode: 404): " + ex.Message, LogDetailLevel.LogRelevant);
                    TestProgramManager.DUTProcessCheckErrCode[i] = ECCS.ERRORCODE_MES_PROCESS_ConnectionLoss;
                    retVal = -1;
                }

                if (!permission_info.ToUpper().Contains("PASS")||!!permission_info.ToUpper().Contains("IGNORE"))
                {
                    
                    if (permission_info.Contains("9100"))
                    {
                        TestProgramManager.DUTProcessCheckErrCode[i] = ECCS.ERRORCODE_MES_PROCESS_BarcodeInvalidError;
                    }
                    else if (permission_info.Contains("9101"))
                    {
                        TestProgramManager.DUTProcessCheckErrCode[i] = ECCS.ERRORCODE_MES_PROCESS_IncorrectProcessStation;
                    }
                    else if (permission_info.Contains("9102"))
                    {
                        TestProgramManager.DUTProcessCheckErrCode[i] = ECCS.ERRORCODE_MES_PROCESS_MESSTOP;
                    }
                    else if (permission_info.Contains("9103"))
                    {
                        TestProgramManager.DUTProcessCheckErrCode[i] = ECCS.ERRORCODE_MES_PROCESS_MESLOCK;
                    }
                    else
                    {
                        TestProgramManager.DUTProcessCheckErrCode[i] = ECCS.ERRORCODE_SHOPFLOOR_PROCESS_ERROR;
                    }
                ...
                if (permission_info.ToLower().Contains("pass"))
                {
                    ...
                    Logger.PrintLog(this, $"DUT# {dut_index + 1}: {permission_info}", LogDetailLevel.LogRelevant);
                    retVal = 0;
                }
                else
                {
                    ...
                    MTKTestProgram_OnShopfloorPermissionCheckFail(dut_index);
                    retVal = 1;
                ...
                if (!shopfloor_permission[dut_index]&&(!DUTInfoDataGridView.Rows[i].Cells["Serial Port"].Value.ToString().Contains("Configure")))
                {
                    ...
                    Logger.PrintLog(this, "DUT# " + (dut_index + 1).ToString() + ": " + permission_info, LogDetailLevel.LogRelevant);
        ...
        public bool UploadResult()
        {
            ...
            {
                MessageBox.Show(ex.Message, "Shopfloor m_SFCS Create Error");

                Logger.PrintLog(this,ex.Message,LogDetailLevel.LogRelevant);
            }
            finally
            {
                //MessageBox.Show("Shopfloor m_SFCS Create Error");
                //if (CyBLEMTKRobotServer.IsGlobalServerActive())
                //{
                //    CyBLEMTKRobotServer.gServer.TestResultsUploadedEvent.Set();
                //}
            ...
            Logger.PrintLog(this, string.Format("Current SFCS ({0}) initialization is completed.", m_SFCS.GetType()), LogDetailLevel.LogRelevant);
        ...
        private void _UploadResult()
        {
            ...
            for (int i = 0; i < DUTInfoDataGridView.Rows.Count; i++)
            {
                ...
                {
                    errorcode = errcode_cal.ReturnFinalDUTOverallSFCSErrCodeforDUT(ProgramStatus[DUT_no],MTKTestProgram.DUTOverallSFCSErrCode[DUT_no], true, DUT_no);

                }
                else
                {
                    errorcode = errcode_cal.ReturnFinalDUTOverallSFCSErrCodeforDUT(ProgramStatus[DUT_no], MTKTestProgram.DUTOverallSFCSErrCode[DUT_no], shopfloor_permission[DUT_no], DUT_no);
                }


                if (errorcode == ECCS.ERRORCODE_SHOPFLOOR_PROCESS_ERROR || errorcode == ECCS.ERRORCODE_MES_PROCESS_BarcodeInvalidError
                    || errorcode == ECCS.ERRORCODE_MES_PROCESS_IncorrectProcessStation|| errorcode == ECCS.ERRORCODE_MES_PROCESS_MESLOCK || errorcode == ECCS.ERRORCODE_MES_PROCESS_MESSTOP)
                {
                ...
                //patch for the secenario when test program has no final programming, DUTInfoDataGridView Status will be wrongly set.
                DUTTestResultToShopfloor = DUTInfoDataGridViewStatusCorrectionByErrorCode(DUTTestResultToShopfloor, errorcode, DUT_no);
                ...
                if ((m_SFCS.SqlConnected) || SupervisorModeMenuItem.Checked || m_SFCS.GetType().ToString().ToUpper().Contains("ROBOTSERVER") || CyBLE_MTK_Application.Properties.Settings.Default.SFCSInterface.ToLower().Contains("local"))
                {
                    ...
                    {
                        MessageBox.Show(m_SFCS.GetType().ToString().Substring(22) + " MES UploadTestResult ERROR (ErrCode: 404): " + ex.Message);
                        Logger.PrintLog(this, m_SFCS.GetType().ToString().Substring(22) + " MES UploadTestResult ERROR (ErrCode: 404): " + ex.Message, LogDetailLevel.LogRelevant);
                    }
                }
                else
                {
                ...
                if (!isSQLconnected && !m_SFCS.GetType().ToString().ToUpper().Contains("LOCAL"))
                {
                    MTKTestProgram_OnShopfloorServerConnectionCheckFail(i);
                }
        ...
        private string DUTInfoDataGridViewStatusCorrectionByErrorCode(string dUTTestResultToShopfloor, ushort errorcode, int dut_no)
        {
            if (DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Value.ToString().ToUpper() != "PASS" &&
                DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Value.ToString().ToUpper() != "FAIL" &&
                DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Value.ToString().ToUpper() != "IGNORE" &&
                DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Value.ToString().ToUpper() != "ERROR")
            {
                //Logger.PrintLog(this, $"Caution: CurrentDUT {CurrentDUT} Compeleted Status is {DUTInfoDataGridView.Rows[CurrentDUT].Cells["Status"].Value.ToString()} which is not completed.", LogDetailLevel.LogRelevant);

                if (errorcode == ECCS.ERRORCODE_ALL_PASS)
                {
                    this.Invoke(new MethodInvoker(() => DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Style = new DataGridViewCellStyle { ForeColor = Color.Green, BackColor = Color.LightGreen }));
                    this.Invoke(new MethodInvoker(() => DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Value = "PASS"));
                    
                }
                else if (errorcode == ECCS.ERRORCODE_DUT_NOT_TEST)
                {
                    this.Invoke(new MethodInvoker(() => DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Style = new DataGridViewCellStyle { ForeColor = Color.Gray, BackColor = Color.LightGray }));
                    this.Invoke(new MethodInvoker(() => DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Value = "IGNORE"));
                }
                else
                {
                    if (!CyBLE_MTK.shopfloor_permission[dut_no])
                    {
                        MTKTestProgram_OnShopfloorPermissionCheckFail(dut_no);
                    }
                    else
                    {
                        this.Invoke(new MethodInvoker(() => DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Style = new DataGridViewCellStyle { ForeColor = Color.Red, BackColor = Color.Pink }));
                        this.Invoke(new MethodInvoker(() => DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Value = "FAIL"));
            ...
            return DUTInfoDataGridView.Rows[dut_no].Cells["Status"].Value.ToString();
        }
        ...
        private void mTKCurrentMeasurementBoardV20SettingsToolStripMenuItem_Click(object sender, EventArgs e)
        {
            CyBLE_Current_Test_OnCurBrd cyBLE_Current_Test_OnCurBrd_Dialog = new CyBLE_Current_Test_OnCurBrd();

            cyBLE_Current_Test_OnCurBrd_Dialog.ShowDialog();
        ...
        private MTKRecord FillinRecord(int CurrentDUT, string SerialNumber, string Model, string TesterID, UInt16 errorcode, string SocketId, string TestResult, string TestStation, string MFI_ID, string TestLog, string remarks)
        {
            ...
            if (ProgAllErr != null && ProgAllErr.Count != 0)
            {
                ...
                {
                    if (ProgAllErr[CurrentDUT] == MTKTestError.IgnoringDUT || ProgAllErr[CurrentDUT] == MTKTestError.ProgrammerNotConfigured || (!DUTsTestFlag[CurrentDUT]&& CyBLE_MTK.shopfloor_permission[CurrentDUT]))
                    {
